...
from FlameMusic import app, BOT_USERNAME, dbb, SUDOERS
import os
import yt_dlp
...
from FlameMusic import dbb, app, BOT_USERNAME, BOT_ID, ASSID, ASSNAME, ASSUSERNAME, ASSMENTION
from FlameMusic.FlameMusicUtilities.tgcallsrun import (music, convert, download, clear, get, is_empty, put, task_done, smexy)
...
from FlameMusic.FlameMusicUtilities.helpers.paste import paste
from FlameMusic.FlameMusicUtilities.tgcallsrun import (music, clear, get, is_empty, put, task_done)
from FlameMusic.FlameMusicUtilities.database.queue import (is_active_chat, add_active_chat, remove_active_chat, music_on, is_music_playing, music_off)
from FlameMusic.FlameMusicUtilities.database.playlist import (get_playlist_count, _get_playlists, get_note_names, get_playlist, save_playlist, delete_playlist)
from FlameMusic.FlameMusicUtilities.database.assistant import (_get_assistant, get_assistant, save_assistant)
from FlameMusic.FlameMusicUtilities.helpers.inline import (play_keyboard, search_markup, play_markup, playlist_markup, audio_markup)
from FlameMusic.FlameMusicUtilities.helpers.inline import play_keyboard, confirm_keyboard, play_list_keyboard, close_keyboard, confirm_group_keyboard
from FlameMusic.FlameMusicUtilities.tgcallsrun import (music, convert, download, clear, get, is_empty, put, task_done, smexy)
from FlameMusic.FlameMusicUtilities.database.queue import (is_active_chat, add_active_chat, remove_active_chat, music_on, is_music_playing, music_off)
from FlameMusic.FlameMusicUtilities.database.onoff import (is_on_off, add_on, add_off)
from FlameMusic.FlameMusicUtilities.database.blacklistchat import (blacklisted_chats, blacklist_chat, whitelist_chat)
from FlameMusic.FlameMusicUtilities.database.gbanned import (get_gbans_count, is_gbanned_user, add_gban_user, add_gban_user)
from FlameMusic.FlameMusicUtilities.database.theme import (_get_theme, get_theme, save_theme)
from FlameMusic.FlameMusicUtilities.database.assistant import (_get_assistant, get_assistant, save_assistant)
from FlameMusic.config import DURATION_LIMIT, ASS_ID
from FlameMusic.FlameMusicUtilities.helpers.decorators import errors
from FlameMusic.FlameMusicUtilities.helpers.filters import command
from FlameMusic.FlameMusicUtilities.helpers.gets import (get_url, themes, random_assistant, ass_det)
...
from FlameMusic.FlameMusicUtilities.helpers.inline import (play_keyboard, search_markup, play_markup, playlist_markup)
import requests
from pyrogram.types import (
    CallbackQuery,
    InlineKeyboardButton,
...
from FlameMusic import aiohttpsession as session
...
"""
    ...
    if CallbackQuery.from_user.id != int(user_id):
        await CallbackQuery.answer(
            "Yᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ ᴛᴏ ᴄʟᴏsᴇ ᴛʜɪs ᴍᴇɴᴜ", show_alert=True
        )
        return
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            await CallbackQuery.message.reply(
                f" Sᴏɴɢ Pᴀᴜsᴇᴅ ʙʏ {rpk}!", reply_markup=play_keyboard
            )
            await CallbackQuery.message.delete()
        else:
            await CallbackQuery.answer(f"Nᴏᴛʜɪɴɢ ɪs ᴘʟᴀʏɪɴɢ!", show_alert=True)
            return
    else:
        await CallbackQuery.answer(f"Nᴏᴛʜɪɴɢ ɪs ᴘʟᴀʏɪɴɢ ɪɴ Mᴜsɪᴄ!", show_alert=True)
...
async def resumevc(_, CallbackQuery):
    ...
    if not a.can_manage_voice_chats:
        return await CallbackQuery.answer(
            """
Yᴏᴜ ᴅᴏ ɴᴏᴛ ʜᴀᴠᴇ ᴛʜᴇ ɴᴇᴄᴇssᴀʀʏ ᴘᴇʀᴍɪssɪᴏɴs ᴛᴏ ᴘᴇʀғᴏʀᴍ ᴛʜɪs ᴀᴄᴛɪᴏɴ.
...
""",
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            await CallbackQuery.answer(
                "I ᴅᴏɴ'ᴛ ᴛʜɪɴᴋ ɪғ sᴏᴍᴇᴛʜɪɴɢ ɪs ᴘᴀᴜsᴇᴅ ɪɴ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ",
                show_alert=True,
            )
            return
        else:
            ...
            rpk = "[" + user_name + "](tg://user?id=" + str(user_id) + ")"
            await CallbackQuery.message.reply(
                f"🎧 Sᴏɴɢ Cᴏɴᴛɪɴᴜᴇᴅ ʙʏ  {rpk}!", reply_markup=play_keyboard
            )
            await CallbackQuery.message.delete()
    else:
        await CallbackQuery.answer(f"Nᴏᴛʜɪɴɢ ɪs ᴘʟᴀʏɪɴɢ!", show_alert=True)
...
""",
    ...
    if await is_active_chat(chat_id):
        ...
        if is_empty(chat_id):
            ...
            await CallbackQuery.message.reply(
                f"""
**Sᴋɪᴘ Bᴜᴛᴛᴏɴ Usᴇᴅ Bʏ** {rpk}

Nᴏ ᴍᴏʀᴇ sᴏɴɢs ɪɴ Qᴜᴇᴜᴇ
...
async def stopvc(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        ...
        user_name = CallbackQuery.from_user.first_name
        rpk = "[" + user_name + "](tg://user?id=" + str(user_id) + ")"
        await CallbackQuery.message.reply(f"🎧 Sᴏɴɢ Sᴛᴏᴘᴘᴇᴅ ʙʏ  {rpk}!")
    else:
...
Personal Playlist Playing."""
    ...
    if str(smex) == "group":
        ...
        if not _playlist:
            return await CallbackQuery.answer(f"Your Group Has No Playlist On Servers. Try Adding AnjelMusics In Playlist.", show_alert=True)
        else:
            await CallbackQuery.message.delete()
            logger_text=f"""Starting Playlist
...
async def group_playlist(_,CallbackQuery):
    ...
    count = 0
    if not _count:
        sex = await CallbackQuery.message.reply_text("Welcome To FlameMusic's Playlist Feature.\n\nGenerating Your Group's Playlist In Database...Please wait.")
...
async def pla_playylistt(_,CallbackQuery):
    ...
    count = 0
    if not _count:
        sex = await CallbackQuery.message.reply_text("**Welcome To AnjelMusic's Playlist Feature.**\n\n**Generating Your Playlist In Database...Please wait.**")
        await asyncio.sleep(2)
        await sex.delete()
    else:
        for smex in _count:
...
async def cbgroupdel(_,CallbackQuery):  
    ...
    _playlist = await get_note_names(CallbackQuery.message.chat.id)                                    
    if not _playlist:
        return await CallbackQuery.message.reply_text("Group has no Playlist on AnjelMusic's Server")
    else:
        titlex = []
        for note in _playlist:
...
async def delplcb(_,CallbackQuery): 
    ...
    _playlist = await get_note_names(CallbackQuery.from_user.id)                                    
    if not _playlist:
        return await CallbackQuery.message.reply_text("You have no Playlist on FlameMusic's Server")
    else:
        titlex = []
        for note in _playlist:
